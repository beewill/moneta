...
module MonetaHelpers
  ...
  module InstanceMethods
    ...
    # advance to the moment just after a tick. 1e-3 is needed in some
    # environments (JRuby) to be able to pass the "not in earlier half" test.
    def advance_next_tick
      tick = t_res
      offset = time_till_next(tick) + 1e-3
      advance offset
      raise "not in earlier half of time" unless Time.now.to_f % tick < tick / 2.0
    end
  end
end
